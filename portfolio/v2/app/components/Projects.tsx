...
const Projects = () => {
  ...
  const projectsTranslations = t('projects.list', { returnObjects: true }) as Array<{
    title: string;
    description: string;
  }>;
  ...
  const projects = [
    {
      title: projectsTranslations[0]?.title || "RenoSmarter.ai – Voiceflow Cost Estimator",
      description: projectsTranslations[0]?.description || "AI-driven home renovation cost estimation app built with Voiceflow, Airtable, and Python. Enhanced pricing logic from 60% to 81% accuracy and boosted completion rates by 40%.",
    ...
    {
      title: projectsTranslations[2]?.title || "Aveem University System",
      description: projectsTranslations[2]?.description || "Full ERP for academic operations across finance, grading, and notifications for 2,000+ users. Built with React, Django, and PostgreSQL, deployed on AWS EC2.",
    ...
    {
      title: projectsTranslations[4]?.title || "Spotter – Trip Planner App",
      description: projectsTranslations[4]?.description || "AI-assisted travel planner that helps users create, organize, and visualize trips interactively. Built with React and deployed on Vercel.",
    ...
    {
      title: projectsTranslations[5]?.title || "Bookify – AI Book Recommendation App",
      description: projectsTranslations[5]?.description || "Book discovery and recommendation web app with real-time search across 10,000+ titles. Built using Django and React.",
    ...
    {
      title: projectsTranslations[6]?.title || "Sentifeel – AI Sentiment Classifier",
      description: projectsTranslations[6]?.description || "GPT-4-powered web classifier achieving 90% accuracy on 5K+ text entries. Built with React for a sleek and fast UI.",
    ...
    {
      title: projectsTranslations[7]?.title || "Readme Converter – Markdown to PDF",
      description: projectsTranslations[7]?.description || "Developer-focused SPA for converting markdown to PDF/HTML with live preview and export capabilities.",
    ...
    {
      title: projectsTranslations[8]?.title || "Personalized AI Chat Assistant",
      description: projectsTranslations[8]?.description || "Conversational AI assistant built with React and OpenAI APIs. Supports context awareness and multi-turn interactions.",
    ...
    {
      title: projectsTranslations[9]?.title || "Journal PWA – Offline Notes App",
      description: projectsTranslations[9]?.description || "Progressive Web App for journaling and note-taking, supporting offline sync, secure storage, and a clean mobile interface.",
            ...
            >
              ...
              <div className="z-10 sm:order-2 sm:col-span-6 m-4">
                <h3 className="font-medium leading-tight text-slate-200">
                  {project.title}
                  <span className="ml-2 text-xs text-teal-300">{t('projects.preview')}</span>
                </h3>
